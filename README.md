...
新增的 `backtest.py` 支持以下七个指数：国证成长100（980080）、国证价值100（980081）、中证红利低波（H30269）、中证科创成长（000690）、国证创业板成长（399667）、国证自由现金流（980092）、中证沪深港黄金产业股票（931238）。黄金资产使用黄金产业股票指数，不是国际现货黄金价格。如果官网提供对应全收益指数，程序优先使用全收益指数并单独缓存，否则自动回退到价格指数。当前已配置的全收益代码包括：480080、480081、H20269、480092。默认使用全部指数，也可以只选择部分指数：
